...
FUS_values = np.load('fus_target_mouse_FUS.npy')
TARDBP_values = np.load('fus_target_mouse_TARDBP.npy')

# Define the CSV file path and chunk size
csv_file_path = 'binned_FUS_mouse_target_genes_sequences.csv'
...
for i, chunk in enumerate(reader):
    ...
    if i == 0:
        ...
        chunk.to_csv('augmented_binned_FUS_mouse_target_genes_sequences.csv', index=False, mode='w')
    else:
        # Append without headers for subsequent chunks
        chunk.to_csv('augmented_binned_FUS_mouse_target_genes_sequences.csv', index=False, mode='a', header=False)
